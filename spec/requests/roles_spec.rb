...
RSpec.describe "/roles", type: :request do
  ...
  let(:valid_attributes) { {role: "Role1"} }
  let(:more_valid_attributes) { {role: "Role2"} }
  let(:invalid_attributes) { {role: ""} }
  let(:same_attributes) { {role: "Role1"} }
  ...
  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) { {role: "Role2"} }

      it "updates the requested role" do
        role = Role.create! valid_attributes
        patch role_url(role), params: { role: new_attributes }
        role.reload
        expect(role.role).to eq(new_attributes[:role])
      end
    ...
    context "with same parameters" do
      ...
      it "renders a response with 422 status (i.e. to display the 'edit' template)" do
        Role.create! valid_attributes
        role = Role.create! more_valid_attributes
        patch role_url(role), params: { role: same_attributes }
        expect(response).to have_http_status(:unprocessable_entity)
      end
