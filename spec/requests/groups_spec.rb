...
RSpec.describe "/groups", type: :request do
  ...
  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) { {group: "Group2"} }

      it "updates the requested group" do
        group = Group.create! valid_attributes
        patch group_url(group), params: { group: new_attributes }
        group.reload
        expect(group.group).to eq(new_attributes[:group])
      end
    ...
    context "with same parameters" do
      ...
      it "renders a response with 422 status (i.e. to display the 'edit' template)" do
        Group.create! valid_attributes
        group = Group.create! more_valid_attributes
        patch group_url(group), params: { group: same_attributes }
        expect(response).to have_http_status(:unprocessable_entity)
      end
